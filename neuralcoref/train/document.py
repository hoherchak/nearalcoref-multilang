...
def _extract_from_sent(doc, span, blacklist=True, debug=False):
    ...
    """
    keep_tags = ["NOUN", "PRON", "PROPN", "DET", "NUM", "ADJ"]
    cache = {}

    # Utility to remove bad endings
    def get_token_mentions(token):
        if token.i in cache:
            return cache[token.i]

        spans = []
        if not(
                token.pos_ not in keep_tags or
                token.lower_ in ['»', '"', '»'] or
                token.dep_.split(':')[0] in ["compound", "flat"]
        ):
            spans.append(doc[token.i: token.i + 1])
            if debug:
        ...
        while stack:
            ...
            for child in node.rights:
                if child.dep_.split(':')[0] in ["det", "compound", "appos", "flat"]:
                    get_token_mentions(child)
                    new_rights = set()
                    for span in cache[child.i]:
                        right = span[-1].i + 1
                        for old_right in rights:
                            new_rights.add(max(right, old_right))
                    rights = list(new_rights)
                    stack.append(child)
        stack = [token]
        while stack:
            node = stack.pop()
            for child in node.lefts:
                if child.dep_.split(':')[0] in ["det", "compound", "appos", "flat"]:
                    ...
                    for span in cache[child.i]:
                        left = span[0].i
                        for old_left in lefts:
                            new_lefts.add(min(left, old_left))
                    lefts = list(new_lefts)
                    stack.append(child)

        for child in token.rights:
            if child.dep_.split(':')[0] in ["nmod", "amod", "nummod", "parataxis"] or child.lower_ in ['«', '»']:
                _right = child.i + 1
                for subchild in child.conjuncts:
                    _right = max(_right, subchild.i + 1)
                rights.append(_right)
                get_token_mentions(child)
                for span in cache[child.i]:
                    rights.append(span[-1].i + 1)

        for child in token.lefts:
            if child.dep_.split(':')[0] in ["nmod", "amod", "nummod", "parataxis"] or child.lower_ in ['«', '»']:
                _left = child.i
                for subchild in child.conjuncts:
                    _right = min(_left, subchild.i)
                lefts.append(_left)
                get_token_mentions(child)
                for span in cache[child.i]:
                    lefts.append(span[0].i)
        ...
        for left_ in lefts:
            for right_ in rights:
                if left_ != right_:
                    spans.append(doc[left_:right_])
                    if debug:
                        print(f"Add modified phrase {doc[left_:right_]}")

        if token.dep_ in ["nsubj"]:
            for child in token.head.rights:
                if child.dep_.split(':')[0] in ["obl"]:
                    _right = child.i + 1
                    for subchild in child.conjuncts:
                        _right = max(_right, subchild.i + 1)
                    spans.append(doc[token.i:_right])
                    get_token_mentions(child)
                    for span in cache[child.i]:
                        spans.append(doc[token.i:span[-1].i + 1])
            for child in token.head.lefts:
                if child.dep_.split(':')[0] in ["obl"]:
                    _left = child.i
                    for subchild in child.conjuncts:
                        _right = min(_left, subchild.i)
                    spans.append(doc[_left:token.i + 1])
                    get_token_mentions(child)
                    for span in cache[child.i]:
                        spans.append(doc[span[0].i:token.i + 1])

        left = min(lefts)
        right = max(rights)
        spans.append(doc[left:right])
        if debug:
            print(f"Add full phrase {doc[left:right]}")
    ...
    for token in span:
        ...
        if token.pos_ not in keep_tags or token.lower_ in ['»', '"', '»'] or token.dep_.split(':')[0] in ["compound", "flat"]:
            if debug:
                print("Skip")
            continue

        mentions_spans += get_token_mentions(token)

        left = token.i
        right = token.i + 1

        for span in cache[token.i]:
            rights = [span[-1].i + 1]
            lefts = [span[0].i]
            for child in token.conjuncts:
                get_token_mentions(child)
                for new_span in cache[child.i]:
                    rights.append(new_span[-1].i + 1)
                    lefts.append(new_span[0].i)
            lefts = list(set(lefts))
            rights = list(set(rights))
            for left_ in lefts:
                for right_ in rights:
                    if left_ != right_:
                        mentions_spans.append(doc[left_:right_])
                        if debug:
                            print(f"Add modified phrase {doc[left_:right_]}")
        ...
        mentions_spans.append(doc[left:right])
        if debug:
            print(f"Add full phrase {doc[left:right]}")
        # raise Exception()
