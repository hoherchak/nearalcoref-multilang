...
class ConllDoc(Document):
    ...
    def get_conll_spacy_lookup(self, conll_tokens, spacy_tokens, debug=False):
        ...
        for c_tok in c_iter:
            ...
            assert len(c_lookup), f"Unmatched conll and spacy tokens: {conll_tokens} <> {list(spacy_tokens)}"
...
class ConllCorpus(object):
    ...
    def read_corpus(self, data_path, model=None, debug=False):
        ...
        snlp = stanza.Pipeline(lang=model, dir=os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))), 'models'))
...
if __name__ == "__main__":
    ...
    CORPUS = ConllCorpus(
        n_jobs=args.n_jobs, gold_mentions=args.gold_mentions, blacklist=args.blacklist, embed_path='coref_uk_helpers/ru/'
    )
